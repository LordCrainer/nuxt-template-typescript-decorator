...
export default {
  ...
  head: {
    titleTemplate: '%s - nuxt-template-typescript-decorator',
    title: 'nuxt-template-typescript-decorator',
    meta: [
      { charset: 'utf-8' },
  ...
  build: {
    ...
    extend(config: any) {
      ...
      config.resolve.alias['@app'] = joinSrc('app')
      config.resolve.alias['@ui'] = joinSrc('ui')
      config.resolve.alias['@@/'] = joinSrc('')
    },
  },
}
